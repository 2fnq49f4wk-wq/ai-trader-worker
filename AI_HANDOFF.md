...
- Status: **V33.351 — 세션 창을 한 곳으로 모으고(A-2) 조기폐장·지연개장을 넣었다(A-8).**
  게이트 111종 전부 통과.
- Branch: `main`

### A-2 — 세션 창이 여러 벌이었다
같은 창(미국 pre 420~570 / post 960~1200, 한국 pre 480~540 / post 930~1200)이 코드 곳곳에
숫자로 박혀 있었다. 실제로 Codex V33.346 이 NXT 체결창을 시세 경로에만 반영해 거래 판정이
옛 창을 들고 있었다 — 두 층이 서로 다른 시각을 믿는 상태였다.
→ `MARKET_HOURS` + `marketWindows(market, now)` + `marketSessionNow(market, now)` 한 곳.
  세션을 보는 함수 15곳이 전부 여기를 본다. 전부 `now` 를 받게 해서 검사 가능해졌다.
→ **평범한 한 주 20,160분 전수 대조 차이 0건** — 평범한 날 동작은 한 분도 안 바뀌었다.

### A-8 — 창은 날마다 같지 않다
`MARKET_HOURS_SPECIAL` 표를 추가했다.
- 미국 반장(13:00 ET 마감): **2026-11-27 · 2026-12-24** — 종전엔 그 3시간을 정규장으로 믿고
  얼어붙은 종가로 거래하면서 야후가 주는 장후 체결가를 normalizeExtUS 가 지웠다.
- 한국 수능일(전 일정 1시간 지연): **2026-11-19** — 09:00~10:00 을 정규장으로(실제 휴장),
  15:30~16:00 을 장후로(실제 정규장) 믿었다.
- 길이를 쓰는 계산도 고쳤다 — `sessionElapsedFraction` 분모가 390 고정이었다(반장 210 · 수능 360).

### 기존 게이트 5종을 새 구조에 맞춰 고쳤다
`check-ext-trading` · `check-overmarket` · `check-extquote-rotation` ·
`check-recovery-provenance` · `check-kr-intraday-window`.
그 과정에서 **소스 문자열로 창 숫자를 찾던 검사 3곳을 '답을 보는' 검사로 바꿨다**
(V33.337 교훈 — 문자열이 사라져도 뜻은 그대로였다).
`check-gate-anchors` 와 `check-stale-base` 가 앵커 사망·배선 누락을 잡아 줬다.

### A-9 — 미국 프리마켓 04:00~07:00 을 통째로 버리고 있었다
미국 프리마켓은 실제로 04:00 ET 에 시작하고 야후도 그때부터 preMarketPrice 를 준다.
그 시각은 **17:00~20:00 KST** — 사용자가 화면을 실제로 보는 시간대다. 종전엔 07:00 부터만
세션으로 봐서 `extKeepMaskUS` 가 그 3시간의 장전 값을 지웠다.
→ 관측 창(`pre`)만 04:00 로 넓히고 **거래 창(`preTrade`)은 07:00 그대로** 뒀다.
  그 구간은 호가가 극도로 얇아 A-3·A-4 가 맞는지 아직 모른다 — 보는 것과 거는 것은 다르다.
→ fetch 비용은 그대로다: 거래 창 밖 시간외는 **6분 주기**(안은 종전 2분).
  관측 3시간을 늘리면서 회차는 90 → 30 으로 줄였다.

### A-7 — 휴장 중 남은 시간외 값에 나이가 없었다
금요일 장후 값이 월요일 장전까지 남는데 **지금 값처럼** 보였다.
→ `extAgeTxt` 로 `20분 전`·`3시간 전`·`3일 전`, 모르면 `시각미상`. 신선하면 안 붙는다.

### 배포 뒤 볼 것
0. **17:00~20:00 KST 에 미국 장전 시세가 화면에 뜨는지**(A-9) · fetch 예산이 버티는지.
1. **2026-11-19 · 11-27 · 12-24** — 이번 고침의 첫 시험이다. 그날 세션 전환 시각 확인.
2. **2027-11-18(수능)은 추정 날짜다** — 교육부 발표로 확정되면 고칠 것.
3. **연말에 다음 해 특례일을 채울 것.** 표에 없는 해는 기본 창으로 돌고 조용히 틀린다.
4. NXT 실제 체결창은 아직 시세 경로에만 있다 — 거래 창은 KRX 기준 그대로다.
5. V33.348~350 확인 대기분은 그대로: `LEDGER_DUP` 로그 · Modal 회차 요약의 `상태 저장소 없음` ·
   `[CF]` 편입 건수 · 실거래 표본 35건.
