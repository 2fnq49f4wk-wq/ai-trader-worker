...
- Scope: V33.312 — AI 작동 관제실(.nlv-*) 시각 규칙을 처음부터 다시 썼다(사용자 지시:
  "지금 화면 완전히 폐기하고 spacex 스타일로 전체 디자인 다시해"). V33.311 은 헤더·텔레메트리·
  브리핑만 새로 얹고 기존 배색을 물려받았는데, 이번엔 이 화면에 걸리는 시각 요소 전부
  (배경·헤더·텔레메트리·히어로·깔때기·위원회·로그·파이프라인·후보 그리드)를 새로 그렸다.
