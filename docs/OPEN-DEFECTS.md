...
`B-8`(코드는 고침, 재수확 시점은 사람이) · `C-1` · `F-3` · **`G-2`(문턱이 두 벌 — 아래)**
...
**다음 작업자가 할 일**
1. 승격 시 `trust.accBase`(그때의 무실력 기준선)와 `trust.passedBy`("acc"/"ic"/"acc+ic")를 **기록**한다.
   `passedBy` 는 이미 `trust` 에 넣고 있다 — `accBase` 만 추가하면 된다.
