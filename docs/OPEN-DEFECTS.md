...
>
> - 기준: `main` **8328ce5** 시점 조사, 런타임 **V33.346**
...
**제안 방향** dead 판정 시 응답 최상위 키 목록 또는 본문 앞 200자를 상태에 남긴다.
...
**후보(미검증)** 표본 풀 3배 증가와 시점이 겹친다. 종목당 추론에 들어가는 것들 —
`mlDeepDecide`(`src/index.js:21329`)가 부르는 MIND/GBDT/SEQ/MEMO/부스터 채점, 피처 조립 —
중 무엇이 커졌는지 **격리하지 못했다.** 트리 깊이는 고정이다(`max_depth 4` / `num_leaves 16`).

**제안 방향** `mlDeepDecide` 내부에 구성요소별 시간 계측을 넣어 어느 모델의 채점이 커졌는지 먼저 잰다.
