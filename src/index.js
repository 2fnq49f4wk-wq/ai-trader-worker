// AI Trader Worker - 완전판
// Cron 1분마다 실행, D1 영구 저장, WATCHLIST/설정/로그 API
...
async function executeSell(DB, market, symbol, pos, price, reason, cfg, cash) {
  const feeRate = market === 'us' ? cfg.feeUS : cfg.feeKR;
  const gross = price * pos.qty, fee = gross * feeRate, proceeds = gross - fee;
    ...
    for (const symbol of tickers) {
      ...
      try {
        ...
        if (!data.price || !data.history || data.history.length < 20) {
          skipped++;
          await log(DB, 'SKIP', symbol, `history short (${data.history?.length || 0})`);
          continue;
        }
        ...
        await saveQuote(DB, symbol, market, { price, prevClose, dayPct, rsi, ma, atr });
...
async function handleRequest(request, env) {
  ...
  if (path === '/api/state') {
    const cfg = { ...DEFAULT_CFG, ...(await getState(env.DB, 'cfg', {})) };
    ...
    return Response.json({ cash, positions: { us: positionsUS,​​​​​​​​​​​​​​​​
