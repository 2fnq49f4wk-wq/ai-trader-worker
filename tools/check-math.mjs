...
//   역참조(\1)로 그것만 지목한다 — 넓게 잡아 오탐을 내면 검사는 곧 꺼진다.
{
  ...
  /* [V33.268] ★네 번째 교정 — 이번에도 코드가 아니라 검사가 틀렸다.★
     기하브라운운동의 이토 표류보정 `(r − 0.5·σ·σ)·dt` 가 걸렸다. 이건 분산 지름길이
     아니다: 빼지는 쪽이 '제곱의 평균' 이 아니라 ★이자율★ 이고, 결과는 음수여도 정상이며
     (하락 표류) sqrt 로 가지도 않는다.
     가르는 자는 ★계수 ½★ 이다. 분산 항등식 E[X²]−E[X]² 에서 E[X]² 에 ½ 이 붙는 일은
     없다(붙으면 그건 분산이 아니다). 그래서 계수가 0.5/.5 인 경우만 뺀다 —
     "sqrt 로 가는가" 를 한 줄 정규식으로 보려다 넓게 잡는 것보다 이쪽이 정확하다. */
  const RE = /[\w\)\]]\s*-\s*(?:(?!0?\.5\b)[\w.$]+\s*\*\s*)?([A-Za-z_$][\w.$]*)\s*\*\s*\1\b/g;
