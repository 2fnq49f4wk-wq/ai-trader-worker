...
console.log("\n③~④ 두 손가락 확대 — 있는가 · 붙잡은 자리를 고정하는가 · 한 곳에 적혀 있는가");
{
  ...
  /* [V33.279] 이동은 화면을 미는 게 아니라 ★회전 축(피벗)★ 을 옮긴다 —
     그래야 옮긴 뒤 돌려도 보던 것이 달아나지 않는다("초점이 고정된 느낌" 의 해결). */
  chk(/panWorld\(v\.x-v0\.x, v\.y-v0\.y\);/.test(HV),
    "두 손가락 중심이 움직이면 회전 축까지 같이 옮긴다(확대와 이동을 동시에)", "두 손가락 이동이 없다");
...
console.log("\n⑥~⑦ 성능 — 프레임당 한 번만 그리는가 · 움직일 때 툴팁을 안 만드는가");
{
  ...
  /* [V33.290] 종전엔 `var ttl=(IF&&!lite)?` 라는 ★식 문자열★ 을 그대로 찾았다. 입력 단계를
     시점별로 다시 그리면서 그 변수가 함수(inTip)로 바뀌자, 계약은 그대로인데 검사만 깨졌다.
     ★무엇을 막는가★ 로 묻는다 — 입력 툴팁과 시점 카드 툴팁 둘 다 lite 에서 안 만들어지는가.
     (실제로 한 장도 안 만드는지는 check-seq3d-render 가 렌더해서 센다.) */
  chk(/lite\) return ''/.test(HV) && /\+\(lite\?'':'<title>t'\+t2/.test(HV),
