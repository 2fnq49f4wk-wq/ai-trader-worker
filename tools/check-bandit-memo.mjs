...
console.log("\n④~⑦ MEMO — 잡음 72축 + 신호 3축 데이터로 실제 학습시켜 본다");
{
  ...
  if (model) {
    ...
    console.log("\n⑦ 자가 신호축을 살리고 잡음축을 죽였는가");
    {
      ...
      /* [V33.334] ★개수가 아니라 무게로 본다.★
         종전엔 "가중 0 이 아닌 잡음축 ≤ 3개" 였는데, 이건 축 개수(D)에 따라 흔들린다 —
         피처가 75 → 80 으로 늘자 우연히 살아남는 축이 하나 더 생겨 검사가 깨졌다.
         하지만 그 축들의 가중은 0.02 였다(신호축은 1.00·0.81·0.74). 잡음바닥은 멀쩡했다.
         지켜야 할 것은 "몇 개가 0이 아닌가"가 아니라 ★잡음이 자를 움직이는가★ 다.
         그래서 무시할 수 없는 크기(0.05 초과)를 갖는 잡음축만 센다 —
         V33.273 사고(잡음축이 0.2~0.5 를 받아 유효축 36/75)는 이 잣대로 즉시 걸린다. */
      const NOISE_EPS = 0.05;
      const nzAll = model.scale.filter((_, j) => !SIG.includes(j)).filter(v => v > 0).length;
      const nz = model.scale.filter((_, j) => !SIG.includes(j)).filter(v => v > NOISE_EPS).length;
      chk(nz === 0,
        `잡음축이 자를 못 움직인다(가중 ${NOISE_EPS} 초과 0개 · 0 아닌 축 ${nzAll}개는 최대 ${noiseMax.toFixed(3)})`,
        `★잡음축 ${nz}개가 무시할 수 없는 가중(>${NOISE_EPS})을 갖는다 — 잡음바닥이 안 빠졌다★`);
