...
//   [V33.228] V33.227 이 누출을 고치며 STACKML.featVer 를 4→5 로 올렸다. 옛 판 표본 6,931건이
//   학습에서 빠져 STACK 은 즉시 표본 0 이 됐다.
//
//   ※ [V33.232 정정] 처음엔 "커서가 창 끝이라 소급생성이 영영 막힌다" 고 적었는데 틀렸다.
//     실측이 반증했다 — "[STACK-BF] +600표본 (홀드아웃경로 · 커서 559572, 에폭 682566)".
//     커서는 창(뒤쪽 20% ≈ 37,000행)의 5분의 1 지점이었다. 막힌 적은 없다.
//     되감는 이유는 다른 데 있다: 이어서만 가면 새 판은 창의 남은 뒤쪽만 갖고, 이미 훑은
//     앞쪽은 옛 판 표본으로만 남아 학습에서 빠진다. 되감으면 같은 창 전체를 새 판으로 갖는다.
//
...
//   조회에 어떤 커서가 묶였는지★ 를 본다.
{
  ...
  // (a) 판 표기 없는 옛 커서 + 지금 판 홀드아웃 표본 0 → 되감아야 한다(프로덕션이 갇혀 있던 상태)
  {
    ...
    if (b && b[1] === 0) ok("판 v" + FV + " 표본 0 · 옛 커서(9000) → 홀드아웃 커서를 창 처음으로 되감았다");
    else no("STACK-OOF: 판이 올라갔는데 커서가 " + (b ? b[1] : "?") + " 에 머문다 — 새 판은 창의 남은 뒤쪽만 갖는다");
