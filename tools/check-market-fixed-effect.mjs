...
console.log("\n⑤ 게이트에 값을 대는 자리들이 실제로 시장을 넘기는가");
{
  ...
  chk(/function _mktOfVec\(v\)/.test(S),
    ...
    "시장 판별이 여러 곳에 흩어져 있다");
  /* [V33.294] ★바뀐 폭이 로그에 안 남으면 이 변경을 검증할 수가 없다.★ 실측에서 MEMO 가
     IC +0.0351 → −0.0325 로 뒤집혔는데, MEMO 로그에만 "섞어재면" 이 빠져 있어
     ★시장 절편을 뺀 탓인지 칸막이가 깎은 탓인지 못 갈랐다.★ 적는 자리를 전수로 확인한다. */
  const logs = [["학습기 공통(_miniLogisticTrain)", /시장" \+ _mkN \+ "개 섞어재면 /],
                ["MEMO", /\[섞어재면 " \+ _num\(model\.valICBlockPooled/]];
  for (const [nm, re] of logs)
    chk(re.test(S), `${nm} 로그가 섞어 잰 값을 함께 적는다`,
      `★${nm} 로그에 섞어 잰 값이 없다 — 바뀐 폭을 못 재면 이 변경을 검증할 수 없다★`);
  chk(/"% 블록IC " \+ _num\(model\.valICBlock, 0\)/.test(S),
    "MEMO 로그가 ★게이트가 보는 값★(블록IC)을 적는다 — 다른 통계를 나란히 놓지 않는다",
    "MEMO 로그가 판정에 안 쓰는 IC 를 적는다");
}
