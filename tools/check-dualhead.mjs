/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ■ 배포 후 실측(V33.252, 표본 20,000) — 두 수정의 기여가 갈렸다:
       BEAR 선형 t −0.61 → +3.9 (라벨 정규화만으로 살아남) · 트리 t 5.4 채택 → 블록IC 0.1597
       BULL 선형 t  0.01 → −0.3 (라벨만으로는 실패)      · 트리 t 3.8 채택 → 블록IC 0.0817
     둘 다 다중검정 보정 문턱(2.99/3.08, 동시검정 12종)을 넘었고 위원회에 잠정합류(×0.25)했다.
