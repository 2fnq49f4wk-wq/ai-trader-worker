...
/* ══ [V33.385] ★비교가 성립하는가.★ 학습은 mw 가중으로 하는데 train_acc 를 균등평균으로
   재면 '모델이 일부러 덜 보는 표본' 이 그 숫자를 지배한다. 실측: 학습표본의 97.2% 가
   recency 바닥에 붙어 있다. 그 상태로 검증(최근 240일)과 비교하면 ★다른 문제의 성적표★ 다.
...
ok(/gap = train_acc - acc/.test(PY) && PY.indexOf("train_acc = float((_hit * _mtr)") < PY.indexOf("gap = train_acc - acc"),
   "★판정에 쓰는 격차가 가중값 기준이다★(균등값으로 판정하면 옛 결론이 그대로 돌아온다)");
/* recency 손잡이가 이 규모에서 무슨 일을 하는지 로그가 말하는가 */
/* 문자열이 아니라 ★실제 print★ 인지 본다 — 주석 처리해도 통과했다(돌연변이에서 확인). */
ok(/print\(f"   \[최근성\] 반감기/.test(PY), "recency 가중이 실제로 무엇을 하는지 회차마다 ★찍는다★");
ok(/이름과 달리 최근을 거의 안 당긴다/.test(PY),
   "★바닥에 붙은 표본이 가중의 대부분이면 그렇다고 말한다★ — 손잡이가 규모 변화로 죽은 것을 숨기지 않는다");
