...
console.log("① 복원값이 라이브 조립과 정확히 같은가 (같은 자를 쓰는가)");
{
  ...
  for (const d of days) {
    ...
    for (let i = 0; i < C; i++) full[D - C + i] = cal[M.CAL_FEATS[i]];
    for (let j = 0; j < D - C; j++) full[j] = ((j * 37) % 19) / 7 - 1;   // 앞칸은 아무 값이나(보존만 보면 된다)
    const old = full.slice(0, D - C);
  ...
  chk(tested === days.length && worst === 0,
    "되살린 75칸이 라이브가 만드는 값과 ★완전히 동일★ 하다(오차 0)",
    `되살린 값이 라이브와 다르다(최대 오차 ${worst}) — 근사가 섞였다`);
  // 앞 69칸이 그대로 보존되는지 — 순서가 밀리면 모든 피처의 뜻이 바뀐다.
  const probe = Array.from({ length: D - C }, (_, j) => j + 0.5);
  const got = M._calBackfillX(probe, Date.parse("2026-08-14T14:30:00Z"));
  chk(got && probe.every((v, j) => got[j] === v),
    "앞 69칸이 한 칸도 밀리지 않고 그대로 보존된다", "★앞칸이 밀렸다 — 모든 피처의 뜻이 바뀐다★");
}

console.log("\n② 달력 6종이 featNames 의 꼬리인가 — 아니면 아무것도 안 하는가");
{
  const tail = names.slice(D - C);
  chk(tail.every((n, i) => n === M.CAL_FEATS[i]),
    `달력 6종이 featNames 의 꼬리다 (${tail.join(",")})`,
    `★달력 6종이 꼬리가 아니다 — 소급복원이 앞칸을 오염시킨다★ (${tail.join(",")})`);
  chk(/for \(let i = 0; i < C; i\+\+\) if \(names\[D - C \+ i\] !== CAL_FEATS\[i\]\) return null;/.test(code),
    "복원기가 그 사실을 ★스스로 확인하고★ 아니면 기권한다(손으로 믿지 않는다)",
    "복원기가 꼬리 여부를 확인하지 않는다 — featNames 순서가 바뀌면 조용히 오염된다");
...
console.log("\n③~⑤ 되살리지 않아야 할 것은 되살리지 않는가");
{
  ...
  chk(M._calBackfillX(new Array(D - C - 4).fill(0), ts) === null,
    `65칸(하이킨아시 이전)은 되살리지 않는다 — 가격이 있어야 만드는 값이다`,
    "★65칸을 되살렸다 — 날짜로 못 만드는 값을 지어냈다★");
  chk(M._calBackfillX(new Array(D).fill(0), ts) === null,
    "이미 75칸인 표본은 건드리지 않는다", "이미 현재 판인 표본을 또 늘린다");
  chk(M._calBackfillX(new Array(D - C).fill(0), 0) === null
      && M._calBackfillX(new Array(D - C).fill(0), null) === null,
  ...
  const preTable = M._calBackfillX(new Array(D - C).fill(0), Date.parse("2015-05-05T14:30:00Z"));
