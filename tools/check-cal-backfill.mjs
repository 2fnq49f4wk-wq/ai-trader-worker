...
console.log("\n⑥ 배선 — 서버가 내보내기 직전에 하고, 파이썬은 다시 구현하지 않는가");
{
  ...
  /* ★파이썬이 달력을 다시 계산하면 규칙이 두 곳에 살게 된다 — 그게 이 저장소의 단골 사고다.
     ══ [V33.399] 다만 종전 검사는 ★이름을 말하는 것★ 까지 금지하고 있었다. ═════════════
     금지해야 할 것은 ★계산★ 이지 ★언급★ 이 아니다. 실제로 V33.399 가 "학습구간에서
     fomcKnown=0 인 행이 몇 %냐" 를 찍으려 하자 이 검사가 막았다 — 진단조차 못 하게 되면
     "달력이 학습에서 죽어 있다" 를 영원히 못 본다(그리고 실제로 못 보고 있었다).
     그래서 ★값을 만들어내는 행위★ 만 셋으로 나눠 막는다:
       ⓐ 표(날짜 리터럴)를 파이썬이 들고 있는가
       ⓑ 세 번째 금요일/요일 산술을 파이썬이 하는가
       ⓒ 달력 칸에 ★값을 써 넣는가★ (읽는 것은 진단이라 허용한다)
     ※ ⓒ 가 핵심이다 — 파이썬이 달력 칸을 write 하지 않는 한 규칙은 워커 한 곳에만 산다. */
  chk(!/"20\d\d-\d\d-\d\d"|'20\d\d-\d\d-\d\d'/.test(PY),
    "ⓐ 파이썬에 회의일 같은 날짜 리터럴이 없다 — 표는 워커에만 있다",
    "★파이썬이 날짜 표를 들고 있다 — 워커 표와 갈라진다★");
  chk(!/third.?friday|_thirdFriday|FOMC_DAYS/i.test(PY),
    "ⓑ 세 번째 금요일·FOMC 표 계산이 파이썬에 없다",
    "★파이썬이 달력 날짜를 계산한다 — 두 구현은 언젠가 갈라진다★");
  {
    const CN = ["opexToNext", "opexWeek", "opexQuad", "fomcTo", "fomcSince", "fomcKnown"];
    const bad = [];
    PY.split("\n").forEach(function (ln, i) {
      const code = ln.replace(/#.*$/, "");           // 주석은 언급일 뿐이다
      if (!CN.some(function (n2) { return code.indexOf(n2) >= 0; })) return;
      // 달력 이름이 나오는 ★코드★ 줄에서 X/Xn 칸에 대입하면 그건 값을 만드는 것이다.
      if (/\b(X|Xn|Xs|Xa)\s*\[[^\]]*\]\s*=(?!=)/.test(code)) bad.push(i + 1);
    });
    chk(bad.length === 0,
      "ⓒ 파이썬이 달력 칸에 값을 써 넣지 않는다(읽고 진단만 한다) — 규칙은 워커 한 곳에만 산다",
      "★파이썬이 달력 칸에 값을 쓴다(줄 " + bad.join(", ") + ") — 두 곳이 갈라진다★");
  }
