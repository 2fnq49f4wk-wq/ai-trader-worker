...
- **항상 `main`에서 직렬로 작업한다.** 작업 전 `git pull --ff-only origin main`, 작업 후 검사·커밋까지
  수행하되 사용자의 별도 배포 지시 없이는 push하지 않는다. 결과는 ZIP으로 패키징한다.
...
- 전달 ZIP은 `bash tools/package-codex-delivery.sh <기준-ref> <이름>`으로 만들며 `.env`, 토큰,
  `.wrangler`, Git 메타데이터와 런타임 산출물을 포함하지 않는다.
