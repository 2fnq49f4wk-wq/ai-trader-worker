...
# ══ [V33.384] ★하루 4회 → 2회.★ 사용자 지시: "dnn seq 하루 2회만 돌리고".
#   회차 전체가 T4 박스에서 돌기 때문에(GPU 를 쓰는 건 DNN·SEQ 뿐인데도) 회차 수가
#   그대로 GPU 청구시간이다. 4회 → 2회면 ★정확히 절반★ 이다: 월 $62 → 약 $31.
#   ※ CPU 단계도 같이 절반이 되지만 ★한 바퀴는 오히려 빨라진다★ —
#     굶은 단계가 있으면 V33.377 의 예약이 DNN 시드를 양보시키기 때문이다:
#       시드 6 → 후속 예산 814s/회차 × 4회 = 3,256s/일  (한 바퀴 4,179s → 31시간)
#       시드 2 → 후속 예산 2,054s/회차 × 2회 = 4,108s/일 (한 바퀴 4,179s → ★24시간★)
#     check-cycle-capacity 가 이 계산을 실제로 돌려 확인한다.
CRON = modal.Cron("10 */12 * * *")  # 매 12시간(하루 2회): 00:10 / 12:10 UTC
